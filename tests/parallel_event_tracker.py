...
def main():   
    ...
    for current_start_block in tqdm(range(start_block, end_block + 1, BLOCK_RANGE_SIZE), desc="Processing blocks"):
        # Calculate current end block
        if current_start_block == start_block and start_block % BLOCK_RANGE_SIZE != 0:
            current_end_block = min(first_batch_end - 1, end_block)
        else:
            current_end_block = min(current_start_block + BLOCK_RANGE_SIZE - 1, end_block)
