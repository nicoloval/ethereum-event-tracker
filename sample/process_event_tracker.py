...
class EventTrackerConfig:
    def __init__(self, contract_file, contract_address, event_file, from_block, to_block, append, log_file, output_file, rpc):
        ...
        self.contract_abi_file = contract_file
        self.contract_address = contract_address
        self.event_solidity_file = event_file
...
def main():
    ...
    logger.info(f"Started event tracking for contract: {config.contract_file}, event: {config.event_file}, address: {config.contract_address}, block range: {config.from_block} to {config.to_block}")
